...
class Document(object):
    def __init__(self):
        ...
        self.mention_to_gold = {}
        self.mention_to_cluster = {}

        for c in self.gold:
            for m in c:
                self.mention_to_gold[m] = tuple(c)

        for c in self.clusters:
            for m in c:
                self.mention_to_cluster[m] = tuple(c)



        self.mentions = [1, 2, 3, 4, 5]
        self.mention_to_gold = {m: tuple(self.mention_to_gold[m]) for m in self.mention_to_gold}
        self.reset()
...
if __name__ == '__main__':
    document = Document()
    print(document.mention_to_cluster)
    print(document.gold)
    print(document.mention_to_gold)
    print(document.clusters)
    ev = Evaluator(muc)
    ev.update(document)
    ev.update(document)
    print(ev.get_f1())
    print(muc(document.clusters, document.mention_to_gold))